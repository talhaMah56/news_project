...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run EDA with optional verbose output.")
    parser.add_argument("--verbose", action="store_true", help="Show output and plots")
    ...
    articles_ai_df = datasets.load_dataset("siavava/ai-tech-articles")["train"].to_pandas()

    # Run EDA on individual datasets
    # MIT AI dataset
    mit_df = full_eda(mit_ai_df, verbose=args.verbose, dataset_name="MIT_AI")
    # AI Tech dataset
    ai_df = full_eda(articles_ai_df, verbose=args.verbose, dataset_name="AI_Tech")
    # Concatenate the two DataFrames vertically
    combined_df = pd.concat([mit_df, ai_df], ignore_index=True)

    # Run EDA on the combined dataset
    full_eda(combined_df, verbose=args.verbose, dataset_name="combined_dataset")
